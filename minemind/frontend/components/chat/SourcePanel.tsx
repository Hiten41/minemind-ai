...
export default function SourcePanel({ sources }: { sources: Source[] }) {
  return (
    <aside className="glass-depth-subtle min-h-0 overflow-hidden rounded-[32px] p-5">
      ...
      {sources.length === 0 ? (
        ...
        <div className="mt-5 max-h-[calc(100vh-190px)] space-y-4 overflow-auto pr-1">
          {sources.map((source, index) => (
            <div key={`${source.title}-${source.excerpt}`} className="rounded-[24px] border border-white/10 bg-black/22 p-4">
              <h3 className="font-semibold text-white/82">{evidenceTitle(source.title, index)}</h3>
              <p className="mt-2 text-sm leading-6 text-white/44">{source.excerpt}</p>
